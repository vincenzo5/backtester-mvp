...
try {
    $allCacheFiles = Get-ChildItem -Path "data/cache/*.csv" -ErrorAction SilentlyContinue
...
if ($newFormatCount -lt $expectedCount) {
    ...
    try {
        $finalCount = (Get-ChildItem -Path "data/cache/*.csv" -ErrorAction SilentlyContinue | Where-Object { $_.Name -notmatch "_2017-" } | Measure-Object).Count
        if ($finalCount -eq 0) {
...
Write-Host "      It will use data from ./data/cache/ (shared with Docker)" -ForegroundColor Yellow
Write-Host ""
