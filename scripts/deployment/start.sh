...
NEW_FORMAT_COUNT=$(ls -1 "$PROJECT_ROOT/data"/*.csv 2>/dev/null | grep -v "_2017-" | wc -l | tr -d ' ' || echo "0")
...
try:
    metadata = yaml.safe_load(open('$PROJECT_ROOT/config/markets.yaml'))
...
if [ "$NEW_FORMAT_COUNT" -lt "$EXPECTED_COUNT" ]; then
    ...
    FINAL_COUNT=$(ls -1 "$PROJECT_ROOT/data"/*.csv 2>/dev/null | grep -v "_2017-" | wc -l | tr -d ' ' || echo "0")
...
echo "      It will use data from ./data/ (shared with Docker)"
echo ""
